...
// 定义用户名和密码
#define ONVIF_USERNAME "admin"
#define ONVIF_PASSWORD "admin123"



// 密文认证检查函数，如果不用验证就可以取消
int verify_authentication(struct soap *soap) {


    return SOAP_OK;//已调通过，但先不需要密码验证
    //===========================================
    ...
    if (!username) {
        printf("[ERROR] Authentication failed: no username provided\n");
        return soap->error; // soap_wsse_get_Username已经设置了错误码
    }
    
    printf("[DEBUG] Authentication attempt: username=%s\n", username);
    

    // 验证用户名是否正确
    if (strcmp(username, ONVIF_USERNAME) != 0) {
        printf("[ERROR] Authentication failed: invalid username\n");
        return SOAP_FAULT;
    } else {
        printf("[DEBUG] Authentication username successful\n");
    }

    // 直接验证密码，不使用时间戳验证
    _wsse__UsernameToken *token = soap_wsse_UsernameToken(soap, NULL);
    if (token && token->Password) {
        if (token->Password->__item && !strcmp(token->Password->__item, ONVIF_PASSWORD)) {
            printf("[DEBUG] Authentication Password successful\n");
            return SOAP_OK;
        }
    }
    
    // 验证密码和时间戳，客户端和服务端的时间戳相差不能超过SOAP_WSSE_CLKSKEW
    // if (soap_wsse_verify_Password(soap, ONVIF_PASSWORD)) {
    //     int err = soap->error;
    //     printf("[ERROR] Authentication failed: invalid password\n");
    //     return err; // soap_wsse_verify_Password已经设置了错误码
    // }
    
    printf("[DEBUG] Authentication all successful\n");
    return SOAP_OK;
}



int __tds__GetCapabilities(struct soap *soap, struct _tds__GetCapabilities *req, struct _tds__GetCapabilitiesResponse *resp)
{
    printf("[DEBUG] GetCapabilities called\n");

    // 检查soap指针
    if (!soap) {
        printf("[ERROR] GetCapabilities: soap is NULL\n");
        return SOAP_EOM;
    }
    
    // 检查认证
    if (verify_authentication(soap) != SOAP_OK) {
        printf("[ERROR] GetCapabilities: authentication failed\n");
        return SOAP_FAULT;
    }

    printf("[DEBUG] GetCapabilities: allocating Capabilities\n");
    resp->Capabilities = (struct tt__Capabilities *)soap_malloc(soap, sizeof(struct tt__Capabilities));
    if (!resp->Capabilities) {
        printf("[ERROR] GetCapabilities: failed to allocate Capabilities\n");
        return SOAP_EOM;
    }

    printf("[DEBUG] GetCapabilities: initializing Capabilities\n");
    memset(resp->Capabilities, 0, sizeof(struct tt__Capabilities));

    // 分配Device能力
    printf("[DEBUG] GetCapabilities: allocating Device capabilities\n");
    resp->Capabilities->Device = (struct tt__DeviceCapabilities *)soap_malloc(soap, sizeof(struct tt__DeviceCapabilities));
    if (resp->Capabilities->Device) {
        printf("[DEBUG] GetCapabilities: allocating Device XAddr\n");
        resp->Capabilities->Device->XAddr = (char *)soap_malloc(soap, strlen(g_device_xaddr) + 1);
        if (resp->Capabilities->Device->XAddr) {
            strcpy(resp->Capabilities->Device->XAddr, g_device_xaddr);
            printf("[DEBUG] GetCapabilities: Device XAddr set to %s\n", resp->Capabilities->Device->XAddr);
        } else {
            printf("[ERROR] GetCapabilities: failed to allocate Device XAddr\n");
            return SOAP_EOM;
        }
        // 初始化Device能力的其他字段
        resp->Capabilities->Device->Network = NULL;
        resp->Capabilities->Device->System = NULL;
        resp->Capabilities->Device->IO = NULL;
        resp->Capabilities->Device->Security = NULL;
        resp->Capabilities->Device->Extension = NULL;
    } else {
        printf("[ERROR] GetCapabilities: failed to allocate Device capabilities\n");
        return SOAP_EOM;
    }

    // 分配Media能力
    printf("[DEBUG] GetCapabilities: allocating Media capabilities\n");
    resp->Capabilities->Media = (struct tt__MediaCapabilities *)soap_malloc(soap, sizeof(struct tt__MediaCapabilities));
    if (resp->Capabilities->Media) {
        printf("[DEBUG] GetCapabilities: allocating Media XAddr\n");
        resp->Capabilities->Media->XAddr = (char *)soap_malloc(soap, strlen(g_media_xaddr) + 1);
        if (resp->Capabilities->Media->XAddr) {
            strcpy(resp->Capabilities->Media->XAddr, g_media_xaddr);
            printf("[DEBUG] GetCapabilities: Media XAddr set to %s\n", resp->Capabilities->Media->XAddr);
        } else {
            printf("[ERROR] GetCapabilities: failed to allocate Media XAddr\n");
            return SOAP_EOM;
        }
        
        // 分配StreamingCapabilities（必需字段）
        printf("[DEBUG] GetCapabilities: allocating StreamingCapabilities\n");
        resp->Capabilities->Media->StreamingCapabilities = (struct tt__RealTimeStreamingCapabilities *)soap_malloc(soap, sizeof(struct tt__RealTimeStreamingCapabilities));
        if (resp->Capabilities->Media->StreamingCapabilities) {
            memset(resp->Capabilities->Media->StreamingCapabilities, 0, sizeof(struct tt__RealTimeStreamingCapabilities));
            // 初始化StreamingCapabilities的内部字段
            resp->Capabilities->Media->StreamingCapabilities->RTPMulticast = NULL;
            resp->Capabilities->Media->StreamingCapabilities->RTP_USCORETCP = NULL;
            resp->Capabilities->Media->StreamingCapabilities->RTP_USCORERTSP_USCORETCP = NULL;
            resp->Capabilities->Media->StreamingCapabilities->Extension = NULL;
        } else {
            printf("[ERROR] GetCapabilities: failed to allocate StreamingCapabilities\n");
            return SOAP_EOM;
        }
        
        // 初始化Media能力的其他字段
        resp->Capabilities->Media->Extension = NULL;
    } else {
        printf("[ERROR] GetCapabilities: failed to allocate Media capabilities\n");
        return SOAP_EOM;
    }

        // 分配PTZ能力
    printf("[DEBUG] GetCapabilities: allocating PTZ capabilities\n");
    resp->Capabilities->PTZ = (struct tt__PTZCapabilities *)soap_malloc(soap, sizeof(struct tt__PTZCapabilities));
    if (resp->Capabilities->PTZ) {
        printf("[DEBUG] GetCapabilities: allocating PTZ XAddr\n");
        char ptz_xaddr[256];
        // snprintf(ptz_xaddr, sizeof(ptz_xaddr), "http://%s:%d/onvif/ptz_service", g_local_ip, ONVIF_TCP_PORT);
        strcpy(ptz_xaddr, g_ptz_xaddr);
        resp->Capabilities->PTZ->XAddr = (char *)soap_malloc(soap, strlen(ptz_xaddr) + 1);
        if (resp->Capabilities->PTZ->XAddr) {
            strcpy(resp->Capabilities->PTZ->XAddr, ptz_xaddr);
            printf("[DEBUG] GetCapabilities: PTZ XAddr set to %s\n", resp->Capabilities->PTZ->XAddr);
        } else {
            printf("[ERROR] GetCapabilities: failed to allocate PTZ XAddr\n");
            return SOAP_EOM;
        }
        // 初始化PTZ能力的其他字段
        // resp->Capabilities->PTZ->Extension = NULL;
    } else {
        printf("[ERROR] GetCapabilities: failed to allocate PTZ capabilities\n");
        return SOAP_EOM;
    }
    ...
    printf("[DEBUG] GetCapabilities: completed successfully\n");
    return SOAP_OK;
}



int __trt__GetStreamUri(struct soap *soap, struct _trt__GetStreamUri *req, struct _trt__GetStreamUriResponse *resp)
{
    printf("GetStreamUri called\n");
    
    // 检查认证
    if (verify_authentication(soap) != SOAP_OK) {
        printf("[ERROR] GetStreamUri: authentication failed\n");
        return SOAP_FAULT;
    }

    resp->MediaUri = (struct tt__MediaUri *)soap_malloc(soap, sizeof(struct tt__MediaUri));
    if (!resp->MediaUri) {
        return SOAP_EOM;
    }

    memset(resp->MediaUri, 0, sizeof(struct tt__MediaUri));

    resp->MediaUri->Uri = (char *)soap_malloc(soap, strlen(g_rtsp_uri) + 1);
    if (!resp->MediaUri->Uri) {
        return SOAP_EOM;
    }
    strcpy(resp->MediaUri->Uri, g_rtsp_uri);

    resp->MediaUri->InvalidAfterConnect = 0;
    resp->MediaUri->InvalidAfterReboot = 0;
    resp->MediaUri->Timeout = 0;
...
SOAP_FMAC5 int SOAP_FMAC6  __wsdd__Probe(struct soap* soap, struct wsdd__ProbeType *wsdd__Probe)
{
    ...
    // sprintf(str_tmp, "http://%s:%d/onvif/device_service", g_local_ip, ONVIF_TCP_PORT);
    sprintf(str_tmp, "%s", g_device_xaddr);
    printf("g_device_xaddr:%s\n", g_device_xaddr);
    ...
    soap_send___wsdd__ProbeMatches(soap, NULL, NULL, &wsdd__ProbeMatches);  //soapClient.c provide
    printf("__wsdd__Probe done \n");
	return SOAP_OK;
...
SOAP_FMAC5 int SOAP_FMAC6 __tds__GetServices(struct soap* soap, struct _tds__GetServices *tds__GetServices, struct _tds__GetServicesResponse *tds__GetServicesResponse)
{
    ...
    sprintf(tds__GetServicesResponse->Service[i].XAddr, "http://%s:%d/onvif/device_service", g_local_ip, ONVIF_TCP_PORT);
    ...
    tds__GetServicesResponse->Service[i].XAddr     = (char *)soap_malloc(soap, sizeof(char)* 100);
    sprintf(tds__GetServicesResponse->Service[i].XAddr, "http://%s:%d/onvif/media_service", g_local_ip, ONVIF_TCP_PORT);
    ...
    tds__GetServicesResponse->Service[i].XAddr     = (char *)soap_malloc(soap, sizeof(char)* 100);
    sprintf(tds__GetServicesResponse->Service[i].XAddr, "http://%s:%d/onvif/image_service", g_local_ip, ONVIF_TCP_PORT);

    // 确保有一个 Service 条目是专门给 PTZ 的
    //ptz
    i = 3;
    tds__GetServicesResponse->Service[i].Namespace = (char *)soap_malloc(soap, sizeof(char)* 100);
    strcpy(tds__GetServicesResponse->Service[i].Namespace, "http://www.onvif.org/ver20/ptz/wsdl");
    tds__GetServicesResponse->Service[i].XAddr     = (char *)soap_malloc(soap, sizeof(char)* 100);
    sprintf(tds__GetServicesResponse->Service[i].XAddr, "http://%s:%d/onvif/ptz_service", g_local_ip, ONVIF_TCP_PORT);
...
SOAP_FMAC5 int SOAP_FMAC6 __tds__GetCapabilities(struct soap* soap, struct _tds__GetCapabilities *tds__GetCapabilities, struct _tds__GetCapabilitiesResponse *tds__GetCapabilitiesResponse)
{
    ...
    if (tds__GetCapabilities->Category[0] == tt__CapabilityCategory__Device ||
        ...
        sprintf(tds__GetCapabilitiesResponse->Capabilities->Device->XAddr, "http://%s:%d/onvif/device_service", g_local_ip, ONVIF_TCP_PORT);
        //<Device><Network>
    ...
    if (tds__GetCapabilities->Category[0] == tt__CapabilityCategory__Events ||
        ...
        sprintf(tds__GetCapabilitiesResponse->Capabilities->Events->XAddr, "http://%s:%d/onvif/event_service", g_local_ip, ONVIF_TCP_PORT);
    ...
    if (tds__GetCapabilities->Category[0] == tt__CapabilityCategory__Imaging ||
        ...
        sprintf(tds__GetCapabilitiesResponse->Capabilities->Imaging->XAddr, "http://%s:%d/onvif/image_service", g_local_ip, ONVIF_TCP_PORT);
    }
    ...
    if (tds__GetCapabilities->Category[0] == tt__CapabilityCategory__Media ||
        ...
        sprintf(tds__GetCapabilitiesResponse->Capabilities->Media->XAddr, "http://%s:%d/onvif/media_service", g_local_ip, ONVIF_TCP_PORT);
...
SOAP_FMAC5 int SOAP_FMAC6 __tds__GetNetworkInterfaces(struct soap* soap, struct _tds__GetNetworkInterfaces *tds__GetNetworkInterfaces, struct _tds__GetNetworkInterfacesResponse *tds__GetNetworkInterfacesResponse)
{
    ...
    *(tds__GetNetworkInterfacesResponse->NetworkInterfaces->Info->MTU) = 1500;
    ...
    strcpy(tds__GetNetworkInterfacesResponse->NetworkInterfaces->IPv4->Config->Manual->Address, g_local_ip);
...
{
    printf("---------------------------__tptz__GetConfigurationOptions---------------------------------------\n");
    // 这里逻辑和 GetNodes 非常像，主要是返回 Limits（限位）
    // 先返回 SOAP_OK 让它不报错，后续再细化
    ...
    tptz__GetConfigurationOptionsResponse->PTZConfigurationOptions->PTZTimeout->Min = "PT0S";   // 最小 0秒
    tptz__GetConfigurationOptionsResponse->PTZConfigurationOptions->PTZTimeout->Max = "PT60S";  // 最大 60秒




    // 在 __tptz__GetConfigurationOptions 函数中
    ...
    // 2. 定义 Pan/Tilt 连续速度空间 (这是显示摇杆的关键！)
    ...
    tptz__GetConfigurationOptionsResponse->PTZConfigurationOptions->Spaces->ContinuousPanTiltVelocitySpace->URI = soap_strdup(soap, "http://www.onvif.org/ver10/tptz/PanTiltSpaces/VelocityGenericSpace");
    ...
    // 5. (可选) 定义 Zoom 连续速度空间 (这是显示变倍按钮的关键)
    ...
    tptz__GetConfigurationOptionsResponse->PTZConfigurationOptions->Spaces->ContinuousZoomVelocitySpace->URI = soap_strdup(soap, "http://www.onvif.org/ver10/tptz/ZoomSpaces/VelocityGenericSpace");
    ...
    tptz__GetConfigurationOptionsResponse->PTZConfigurationOptions->Spaces->ContinuousZoomVelocitySpace->XRange->Min = -1.0;
...
SOAP_FMAC5 int SOAP_FMAC6 __trt__GetProfiles(struct soap* soap, struct _trt__GetProfiles *trt__GetProfiles, struct _trt__GetProfilesResponse *trt__GetProfilesResponse)
{
    ...
    *(trt__GetProfilesResponse->Profiles[i].fixed) = xsd__boolean__false_;
    ...
    // 在 __trt__GetProfiles 中
    // 关联 PTZ 配置 (Token 必须和 __tptz__GetConfigurations 里返回的一模一样)
    trt__GetProfilesResponse->Profiles->PTZConfiguration = (struct tt__PTZConfiguration*)soap_malloc(soap, sizeof(struct tt__PTZConfiguration));
    memset(trt__GetProfilesResponse->Profiles->PTZConfiguration, 0, sizeof(struct tt__PTZConfiguration));
    trt__GetProfilesResponse->Profiles->PTZConfiguration->token = soap_strdup(soap, "PTZConfig_1"); 
    trt__GetProfilesResponse->Profiles->PTZConfiguration->NodeToken = soap_strdup(soap, "PTZNode_1");

    trt__GetProfilesResponse->Profiles->PTZConfiguration->UseCount = 1; 
    trt__GetProfilesResponse->Profiles->PTZConfiguration->DefaultContinuousPanTiltVelocitySpace = "http://www.onvif.org/ver10/tptz/PanTiltSpaces/VelocityGenericSpace";
    trt__GetProfilesResponse->Profiles->PTZConfiguration->DefaultContinuousZoomVelocitySpace = "http://www.onvif.org/ver10/tptz/ZoomSpaces/VelocityGenericSpace";

    trt__GetProfilesResponse->Profiles->PTZConfiguration->DefaultAbsolutePantTiltPositionSpace = "http://www.onvif.org/ver10/tptz/PanTiltSpaces/PositionGenericSpace";
    trt__GetProfilesResponse->Profiles->PTZConfiguration->DefaultAbsoluteZoomPositionSpace = "http://www.onvif.org/ver10/tptz/ZoomSpaces/PositionGenericSpace";
    
    trt__GetProfilesResponse->Profiles->PTZConfiguration->DefaultRelativePanTiltTranslationSpace = "http://www.onvif.org/ver10/tptz/PanTiltSpaces/TranslationGenericSpace";
    trt__GetProfilesResponse->Profiles->PTZConfiguration->DefaultRelativeZoomTranslationSpace = "http://www.onvif.org/ver10/tptz/ZoomSpaces/TranslationGenericSpace";

    // 分配速度结构
    trt__GetProfilesResponse->Profiles->PTZConfiguration->DefaultPTZSpeed = (struct tt__PTZSpeed*)soap_malloc(soap, sizeof(struct tt__PTZSpeed));
    memset(trt__GetProfilesResponse->Profiles->PTZConfiguration->DefaultPTZSpeed, 0, sizeof(struct tt__PTZSpeed));
    // 填充 PanTilt 速度
    trt__GetProfilesResponse->Profiles->PTZConfiguration->DefaultPTZSpeed->PanTilt = (struct tt__Vector2D*)soap_malloc(soap, sizeof(struct tt__Vector2D));
    trt__GetProfilesResponse->Profiles->PTZConfiguration->DefaultPTZSpeed->PanTilt->x = 0.5; // 默认速度 x
    trt__GetProfilesResponse->Profiles->PTZConfiguration->DefaultPTZSpeed->PanTilt->y = 0.5; // 默认速度 y
    trt__GetProfilesResponse->Profiles->PTZConfiguration->DefaultPTZSpeed->PanTilt->space = "http://www.onvif.org/ver10/tptz/PanTiltSpaces/VelocityGenericSpace";

    // 填充 Zoom 速度
    trt__GetProfilesResponse->Profiles->PTZConfiguration->DefaultPTZSpeed->Zoom = (struct tt__Vector1D*)soap_malloc(soap, sizeof(struct tt__Vector1D));
    trt__GetProfilesResponse->Profiles->PTZConfiguration->DefaultPTZSpeed->Zoom->x = 0.5;
    trt__GetProfilesResponse->Profiles->PTZConfiguration->DefaultPTZSpeed->Zoom->space = "http://www.onvif.org/ver10/tptz/ZoomSpaces/VelocityGenericSpace";
...
SOAP_FMAC5 int SOAP_FMAC6 __trt__GetVideoEncoderConfigurations(struct soap* soap, struct _trt__GetVideoEncoderConfigurations *trt__GetVideoEncoderConfigurations, struct _trt__GetVideoEncoderConfigurationsResponse *trt__GetVideoEncoderConfigurationsResponse)
{
    ...
    trt__GetVideoEncoderConfigurationsResponse->Configurations[i].SessionTimeout = (char *)soap_malloc(soap, 32);
    if (!trt__GetVideoEncoderConfigurationsResponse->Configurations[i].SessionTimeout) {
        return SOAP_EOM;
    }
    strcpy(trt__GetVideoEncoderConfigurationsResponse->Configurations[i].SessionTimeout, ONVIF_TIME_OUT);
...
SOAP_FMAC5 int SOAP_FMAC6 __trt__GetVideoEncoderConfiguration(struct soap* soap, struct _trt__GetVideoEncoderConfiguration *trt__GetVideoEncoderConfiguration, struct _trt__GetVideoEncoderConfigurationResponse *trt__GetVideoEncoderConfigurationResponse)
{
    ...
    trt__GetVideoEncoderConfigurationResponse->Configuration->SessionTimeout = (char *)soap_malloc(soap, 32);
    if (!trt__GetVideoEncoderConfigurationResponse->Configuration->SessionTimeout) {
        return SOAP_EOM;
    }
    strcpy(trt__GetVideoEncoderConfigurationResponse->Configuration->SessionTimeout, ONVIF_TIME_OUT);
    return SOAP_OK;
}
...
SOAP_FMAC5 int SOAP_FMAC6 __trt__GetSnapshotUri(struct soap* soap, struct _trt__GetSnapshotUri *trt__GetSnapshotUri, struct _trt__GetSnapshotUriResponse *trt__GetSnapshotUriResponse)
{
    ...
    sprintf(trt__GetSnapshotUriResponse->MediaUri->Uri, "http://%s:%d/snap0.jpeg", g_local_ip, 80);
    ...
    trt__GetSnapshotUriResponse->MediaUri->Timeout = (char *)soap_malloc(soap, 32);
    if (!trt__GetSnapshotUriResponse->MediaUri->Timeout) {
        return SOAP_EOM;
    }
    strcpy(trt__GetSnapshotUriResponse->MediaUri->Timeout, ONVIF_TIME_OUT);
...
/** Web service operation '__tds__GetUserRoles' implementation, should return SOAP_OK or error code */
SOAP_FMAC5 int SOAP_FMAC6 __tds__GetUserRoles(struct soap* soap, struct _tds__GetUserRoles *tds__GetUserRoles, struct _tds__GetUserRolesResponse *tds__GetUserRolesResponse)
{
    return 0;
}


 /** Web service operation '__tds__SetUserRole' implementation, should return SOAP_OK or error code */
SOAP_FMAC5 int SOAP_FMAC6 __tds__SetUserRole(struct soap* soap, struct _tds__SetUserRole *tds__SetUserRole, struct _tds__SetUserRoleResponse *tds__SetUserRoleResponse)
{
    return 0;
}

/** Web service operation '__tds__DeleteUserRole' implementation, should return SOAP_OK or error code */
SOAP_FMAC5 int SOAP_FMAC6 __tds__DeleteUserRole(struct soap* soap, struct _tds__DeleteUserRole *tds__DeleteUserRole, struct _tds__DeleteUserRoleResponse *tds__DeleteUserRoleResponse)
{
    return 0;
}
...
/** Web service operation '__tds__UpgradeFirmware' implementation, should return SOAP_OK or error code */
SOAP_FMAC5 int SOAP_FMAC6 __tds__UpgradeFirmware(struct soap* soap, struct _tds__UpgradeFirmware *tds__UpgradeFirmware, struct _tds__UpgradeFirmwareResponse *tds__UpgradeFirmwareResponse)
{
    return 0;
}
...
SOAP_FMAC5 int SOAP_FMAC6 __tptz__GetServiceCapabilities(struct soap* soap, struct _tptz__GetServiceCapabilities *tptz__GetServiceCapabilities, struct _tptz__GetServiceCapabilitiesResponse *tptz__GetServiceCapabilitiesResponse)
{
    printf("---------------------------__tptz__GetServiceCapabilities---------------------------------------\n");
    
    // 检查认证
    if (verify_authentication(soap) != SOAP_OK) {
        printf("[ERROR] __tptz__GetServiceCapabilities: authentication failed\n");
        return SOAP_FAULT;
    }
    ...
    *(tptz__GetServiceCapabilitiesResponse->Capabilities->EFlip) = xsd__boolean__true_;
    tptz__GetServiceCapabilitiesResponse->Capabilities->Reverse = (enum xsd__boolean *)soap_malloc(soap, sizeof(enum xsd__boolean));
    *(tptz__GetServiceCapabilitiesResponse->Capabilities->Reverse) = xsd__boolean__true_;
...
SOAP_FMAC5 int SOAP_FMAC6 __tptz__GetNodes(struct soap* soap, struct _tptz__GetNodes *tptz__GetNodes, struct _tptz__GetNodesResponse *tptz__GetNodesResponse)
{
    printf("---------------------------__tptz__GetNodes---------------------------------------\n");
    
    // 检查认证
    if (verify_authentication(soap) != SOAP_OK) {
        printf("[ERROR] __tptz__GetNodes: authentication failed\n");
        return SOAP_FAULT;
    }
    ...
    // Set node token
    tptz__GetNodesResponse->PTZNode->token = (char *)soap_malloc(soap, sizeof(char) * 20);
    memset(tptz__GetNodesResponse->PTZNode->token, 0, sizeof(char) * 20);
    strcpy(tptz__GetNodesResponse->PTZNode->token, "PTZNode_1");

    // Set node name
    tptz__GetNodesResponse->PTZNode->Name = (char *)soap_malloc(soap, sizeof(char) * 20);
    memset(tptz__GetNodesResponse->PTZNode->Name, 0, sizeof(char) * 20);
    strcpy(tptz__GetNodesResponse->PTZNode->Name, "Main PTZ Node");

    // Set maximum number of presets
    tptz__GetNodesResponse->PTZNode->MaximumNumberOfPresets = 10;

    // Set home supported
    tptz__GetNodesResponse->PTZNode->HomeSupported = xsd__boolean__true_;


    // --- 关键修复：必须先分配 SupportedPTZSpaces 的内存 ---
    tptz__GetNodesResponse->PTZNode->SupportedPTZSpaces = (struct tt__PTZSpaces*)soap_malloc(soap, sizeof(struct tt__PTZSpaces));
    memset(tptz__GetNodesResponse->PTZNode->SupportedPTZSpaces, 0, sizeof(struct tt__PTZSpaces)); // 初始化为0
    // --- Pan/Tilt 绝对位置空间 ---
    ...
    tptz__GetNodesResponse->PTZNode->SupportedPTZSpaces->AbsolutePanTiltPositionSpace->URI = (char*)soap_malloc(soap, 256);
    strcpy(tptz__GetNodesResponse->PTZNode->SupportedPTZSpaces->AbsolutePanTiltPositionSpace->URI, "http://www.onvif.org/ver10/tptz/PanTiltSpaces/PositionGenericSpace");
    ...
    tptz__GetNodesResponse->PTZNode->SupportedPTZSpaces->AbsoluteZoomPositionSpace->URI = (char*)soap_malloc(soap, 256);
    strcpy(tptz__GetNodesResponse->PTZNode->SupportedPTZSpaces->AbsoluteZoomPositionSpace->URI, "http://www.onvif.org/ver10/tptz/ZoomSpaces/PositionGenericSpace");
    ...
    tptz__GetNodesResponse->PTZNode->SupportedPTZSpaces->ContinuousPanTiltVelocitySpace->URI = (char*)soap_malloc(soap, 256);
...
SOAP_FMAC5 int SOAP_FMAC6 __tptz__Stop(struct soap* soap, struct _tptz__Stop *tptz__Stop, struct _tptz__StopResponse *tptz__StopResponse)
{
    ...
    // Print stop command
    printf("Stop PTZ movement\n");
    printf("tptz__Stop: PanTilt - %d\n", *tptz__Stop->PanTilt);
    printf("tptz__Stop: Zoom - %d\n", *tptz__Stop->Zoom);
    return SOAP_OK;
}
...
SOAP_FMAC5 int SOAP_FMAC6 __tptz__GetPresets(struct soap* soap, struct _tptz__GetPresets *tptz__GetPresets, struct _tptz__GetPresetsResponse *tptz__GetPresetsResponse)
{
    printf("---------------------------__tptz__GetPresets---------------------------------------\n");

    // Set preset count
    tptz__GetPresetsResponse->__sizePreset = 2;
    
    // Allocate presets
    tptz__GetPresetsResponse->Preset = (struct tt__PTZPreset *)soap_malloc(soap, sizeof(struct tt__PTZPreset) * 2);
    
    // Preset 1
    memset(&tptz__GetPresetsResponse->Preset[0], 0, sizeof(struct tt__PTZPreset));
    tptz__GetPresetsResponse->Preset[0].token = (char *)soap_malloc(soap, sizeof(char) * 20);
    memset(tptz__GetPresetsResponse->Preset[0].token, 0, sizeof(char) * 20);
    strcpy(tptz__GetPresetsResponse->Preset[0].token, "Preset1");
    tptz__GetPresetsResponse->Preset[0].Name = (char *)soap_malloc(soap, sizeof(char) * 20);
    memset(tptz__GetPresetsResponse->Preset[0].Name, 0, sizeof(char) * 20);
    strcpy(tptz__GetPresetsResponse->Preset[0].Name, "Home Position");

    // Preset 2
    memset(&tptz__GetPresetsResponse->Preset[1], 0, sizeof(struct tt__PTZPreset));
    tptz__GetPresetsResponse->Preset[1].token = (char *)soap_malloc(soap, sizeof(char) * 20);
    memset(tptz__GetPresetsResponse->Preset[1].token, 0, sizeof(char) * 20);
    strcpy(tptz__GetPresetsResponse->Preset[1].token, "Preset2");
    tptz__GetPresetsResponse->Preset[1].Name = (char *)soap_malloc(soap, sizeof(char) * 20);
    memset(tptz__GetPresetsResponse->Preset[1].Name, 0, sizeof(char) * 20);
    strcpy(tptz__GetPresetsResponse->Preset[1].Name, "View Position");
...
SOAP_FMAC5 int SOAP_FMAC6 __tptz__GotoPreset(struct soap* soap, struct _tptz__GotoPreset *tptz__GotoPreset, struct _tptz__GotoPresetResponse *tptz__GotoPresetResponse)
{
    printf("---------------------------__tptz__GotoPreset---------------------------------------\n");

    // Print preset token
    if (tptz__GotoPreset->PresetToken) {
        printf("GotoPreset: Moving to preset %s\n", tptz__GotoPreset->PresetToken);
    }
...
SOAP_FMAC5 int SOAP_FMAC6 __tptz__GetConfigurations(struct soap *soap, struct _tptz__GetConfigurations *tptz__GetConfigurations, struct _tptz__GetConfigurationsResponse *tptz__GetConfigurationsResponse)
{
    printf("[PTZ] GetConfigurations called\n");
    printf("---------------------------__tptz__GetConfigurations---------------------------------------\n");
    ...
    // --- 关键设置 ---
    // Token 必须唯一，且要和 GetProfiles 里的 PTZConfiguration.token 一致
    tptz__GetConfigurationsResponse->PTZConfiguration->token = soap_strdup(soap, "PTZConfig_1");
    tptz__GetConfigurationsResponse->PTZConfiguration->Name = soap_strdup(soap, "PTZConfig_1");
    
    // NodeToken 指向物理节点，通常和 Token 一样即可
    tptz__GetConfigurationsResponse->PTZConfiguration->NodeToken = soap_strdup(soap, "PTZNode_1");

    // 默认空间定义 (ODM 依赖这些字符串)
    tptz__GetConfigurationsResponse->PTZConfiguration->DefaultAbsolutePantTiltPositionSpace = soap_strdup(soap, "http://www.onvif.org/ver10/tptz/PanTiltSpaces/PositionGenericSpace");
    tptz__GetConfigurationsResponse->PTZConfiguration->DefaultRelativePanTiltTranslationSpace = soap_strdup(soap, "http://www.onvif.org/ver10/tptz/PanTiltSpaces/TranslationGenericSpace");
    tptz__GetConfigurationsResponse->PTZConfiguration->DefaultContinuousPanTiltVelocitySpace = soap_strdup(soap, "http://www.onvif.org/ver10/tptz/PanTiltSpaces/VelocityGenericSpace");
...
SOAP_FMAC5 int SOAP_FMAC6 __tptz__GetStatus(struct soap *soap, struct _tptz__GetStatus *tptz__GetStatus, struct _tptz__GetStatusResponse *tptz__GetStatusResponse)
{
    printf("[PTZ] GetStatus called\n");
    printf("-------------------------------__tptz__GetStatus---------------------------------\n");
    ...
    memset(tptz__GetStatusResponse->PTZStatus->Position, 0, sizeof(struct tt__PTZVector));

    // 2. 设置 Pan/Tilt 坐标 (范围 -1.0 到 1.0)
    // 假设初始在中心 (0,0)
    ...
    tptz__GetStatusResponse->PTZStatus->MoveStatus->PanTilt = false;
    tptz__GetStatusResponse->PTZStatus->MoveStatus->Zoom = false;
